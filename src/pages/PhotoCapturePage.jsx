...
function PhotoCapture() {
  ...
  const fetchStudentData = async () => {
    try {
      const response = await fetch(`https://13.126.248.119:5001/api/students/${studentId}`);
      if (response.ok) {
  ...
  const uploadPhoto = async (imageDataUrl) => {
    ...
    try {
      ...
      const uploadResponse = await fetch(`https://13.126.248.119:5001/api/student/upload-student-image/${studentId}`, {
        method: 'POST',
        body: formData,
      });
